...
def load_data():
    ...
    # Replace these with your actual PAT and repo name
    github_pat = "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"
    repo_name = "FOMC_streamlit"
    file_path = "FOMCAnalysis_data.pkl"
    ...
    pickle_content = read_file(file_path)
    if pickle_content is not None:
        buffer = BytesIO(pickle_content)
        nested_dict = pickle.load(buffer)
        return nested_dict
    else:
        return None

data = load_data()
offi = list(data['Filtered_df'].keys())
